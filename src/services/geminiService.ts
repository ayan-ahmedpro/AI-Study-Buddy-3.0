...
const getAI = () => {
  if (!aiInstance) {
    ...
    const isMock = apiKey?.startsWith("YOUR_");
    ...
    if (!apiKey || isMock) {
      ...
      throw new Error("GEMINI_API_KEY is not configured correctly. If you are using a shared link, please ensure the owner has set the API Key in AI Studio Settings > API Keys.");
    }
...
export const analyzeStudyMaterial = async (files: File[], age: number = 20, targetLanguage: string = 'English') => {
  ...
  const modelToUse = hasVideo ? "gemini-3.1-pro-preview" : "gemini-3.5-flash";
