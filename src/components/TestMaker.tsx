...
const TestMaker: React.FC<TestMakerProps> = ({ topics, onClose }) => {
  ...
  const generateTest = async () => {
    ...
    try {
      ...
      alert("Test generation failed. This usually happens when the AI is overloaded. Please try again in 5 seconds.");
    } finally {
      setLoading(false);
    }
  };
            ...
            {loading ? (
              ...
              >
                ...
                <button 
                  ...
                  className="w-full bg-gradient-to-r from-indigo-600 to-purple-700 text-white py-5 rounded-2xl font-black shadow-xl shadow-indigo-100 hover:scale-[1.02] active:scale-95 transition-all text-lg flex items-center justify-center gap-3"
                >
              ...
              >
                ...
                <div className="space-y-4 text-left max-h-48 overflow-y-auto px-4 custom-scrollbar">
                   {(test || []).map((q, i) => (
                      <div key={i} className="flex gap-4 items-start pb-4 border-b border-slate-100 last:border-0 hover:bg-slate-50 p-2 rounded-xl transition-colors">
                         <div className={cn(
