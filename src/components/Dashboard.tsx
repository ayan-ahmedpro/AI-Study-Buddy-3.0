...
const Dashboard: React.FC<DashboardProps> = ({ onBack, onSelectSession, onShowWeaknessDetector }) => {
  ...
  const handleExportReport = async () => {
    try {
      ...
      page.drawText('AI STUDY BUDDY - MASTERY REPORT', {
        x: 50,
        y: height - 50,
        size: 20,
        font,
      ...
      page.drawText(`User ID: ${auth.currentUser?.uid?.substring(0, 8)}...`, {
        x: 50,
        y: height - 80,
        size: 10,
        font: normalFont,
      ...
      // Stats Summary
      page.drawText('OVERALL PERFORMANCE', { x: 50, y: height - 140, size: 14, font });
      page.drawText(`Total Study Sessions: ${totalSessions}`, { x: 50, y: height - 165, size: 12, font: normalFont });
      page.drawText(`Average Mastery Score: ${avgMastery}%`, { x: 50, y: height - 185, size: 12, font: normalFont });

      // Session History table
      page.drawText('SESSION HISTORY', { x: 50, y: height - 230, size: 14, font });
      let yOffset = height - 260;
      
      const tableHeaderY = yOffset;
      page.drawText('Topic', { x: 50, y: tableHeaderY, size: 10, font });
      page.drawText('Date', { x: 300, y: tableHeaderY, size: 10, font });
      page.drawText('Score', { x: 450, y: tableHeaderY, size: 10, font });
      
      yOffset -= 20;
      sessions.slice(-15).reverse().forEach((s, i) => {
        if (yOffset > 50) {
          const dateStr = s.timestamp?.toDate ? s.timestamp.toDate().toLocaleDateString() : 'Recent';
          const scoreStr = `${Math.round((s.quizScore / (s.totalQuestions || 1)) * 100)}%`;
          
          page.drawText(s.topic.substring(0, 40), { x: 50, y: yOffset, size: 10, font: normalFont });
          page.drawText(dateStr, { x: 300, y: yOffset, size: 10, font: normalFont });
          page.drawText(scoreStr, { x: 450, y: yOffset, size: 10, font: normalFont });
          
          yOffset -= 15;
        }
      });

      page.drawText('End of Report - AI Study Buddy by Ayan Ahmed', {
        x: 50,
        y: 40,
        size: 8,
        font: normalFont,
      ...
      saveAs(blob, `AI_Study_Report_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (err) {
