          ...
          <div className="px-6 py-3 border-b border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-950/20 flex items-center gap-4 text-xs">
            ...
            <div className="flex items-center gap-2 overflow-x-auto py-1">
              {(roomData?.activeParticipants || [{ uid: userUid, name: userName, lastSeen: '' }]).map((p, idx) => (
                <div key={p.uid || `participant-${idx}`} className="flex items-center gap-1.5 px-3 py-1 bg-white dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700 text-xs font-semibold shadow-xs">
              ...
              <div className="flex-1 p-4 space-y-3 overflow-y-auto max-h-[300px] md:max-h-none">
                {messages.length === 0 ? (
                  ...
                  messages.map((m) => {
                    ...
                    return (
                      <div key={m.id} className={cn("flex flex-col space-y-1", isMe ? "items-end" : "items-start")}>
