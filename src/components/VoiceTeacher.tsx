...
export default function VoiceTeacher({ initialContext, initialImage, age = 20, onClose }: VoiceTeacherProps) {
  ...
  const getFriendlyErrorMessage = (err: any): string => {
    const errStr = String(err?.message || err?.statusText || err?.status || err?.reason || err || "").toLowerCase();
    if (
      ...
      return "API Quota Limit: The Gemini Live API real-time voice session has hit its rate or billing limits. Real-time audio streaming consumes high token volume. Please try again in a few minutes, or use the standard Live Chat on the main dashboard which is optimized for standard text quotas!";
    }
          ...
          {isConnecting ? (
            ...
            <div className="text-center space-y-4">
              <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto">
                <MicOff className="text-red-600 w-8 h-8" />
              </div>
