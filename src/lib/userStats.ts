...
export const getUserStats = async (user: User | null): Promise<UserStatsData> => {
  ...
  try {
    ...
    if (snap.exists()) {
      ...
      const isPro = Boolean(data.isPro || cachedData?.isPro);
      const stats = {
      ...
      const isPro = Boolean(cachedData?.isPro || localStorage.getItem('guest_is_pro') === 'true');
      const initialData = {
        uid: user.uid,
        email: user.email || '',
    ...
    return {
      analysesUsed: getGuestAnalysesUsed(),
      bonusAnalyses: 0,
      isPro: localStorage.getItem('guest_is_pro') === 'true',
      currentStreak: 0,
      longestStreak: 0,
      lastStudyDate: '',
...
export const setUserProStatusByUid = async (uid: string, isPro: boolean = true): Promise<void> => {
  if (!uid) return;
  try {
...
/**
 * Checks if a given user UID is present in the admins collection.
 */
