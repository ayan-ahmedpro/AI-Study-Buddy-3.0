...
import NotebookInfographic from './components/NotebookInfographic';
...
import DeepThinkingWorkbench from './components/DeepThinkingWorkbench';
import SearchGroundingHub from './components/SearchGroundingHub';
import SourcesPanel from './components/SourcesPanel';
import SourceExcerptModal from './components/SourceExcerptModal';
import NotebookGuide from './components/NotebookGuide';
import StudyCast from './components/StudyCast';
import MindMap from './components/MindMap';
import NotesPanel from './components/NotesPanel';
import NotebooksHome from './components/NotebooksHome';
import UpgradeModal from './components/UpgradeModal';
import { LearningRoadmapModal } from './components/LearningRoadmapModal';
import { SessionEndModal } from './components/SessionEndModal';
import AccountSettingsModal from './components/AccountSettingsModal';
import PublicProgressDashboard from './components/PublicProgressDashboard';
import FlashcardSpacedRepetition from './components/FlashcardSpacedRepetition';
...
import { getUserStats, incrementUserAnalysesUsed, updateStudyStreak, processReferralCode, setUserProStatus, checkIsAdmin, UserStatsData } from './lib/userStats';
...
import { collection, addDoc, serverTimestamp, query, getDocs, doc, setDoc, deleteDoc } from 'firebase/firestore';
...
export default function App() {
  ...
  const [view, setView] = useState<'landing' | 'home' | 'notebooks' | 'dashboard' | 'admin'>('landing');
  ...
  // Save notebooks to local storage for guests
  useEffect(() => {
    if (!currentUser && notebooks.length > 0) {
  ...
  const runNotebookAnalysis = async (nb: Notebook) => {
    ...
    const freeLimit = 5 + currentStats.bonusAnalyses;
    if (!currentStats.isPro && currentStats.analysesUsed >= freeLimit) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (errorMessage.includes("GEMINI_API_KEY") || errorMessage.includes("API key not valid")) {
        appErr.type = 'auth';
      ...
      } else if (errorMessage.includes("QUOTA_EXCEEDED") || errorMessage.includes("429") || errorMessage.includes("RESOURCE_EXHAUSTED")) {
        appErr.type = 'quota';
        appErr.title = 'Neural Capacity Reached';
        appErr.message = 'The AI server is at peak capacity. Take a 1-minute water break while the system recalibrates—I\'ll be ready for you soon!';
      ...
      } else if (errorMessage.includes("token count") || errorMessage.includes("exceeds the maximum number of tokens") || errorMessage.includes("1048576")) {
        appErr.type = 'quota';
  ...
  return (
    <div className="min-h-screen relative bg-[var(--color-study-bg)] dark:bg-[var(--color-study-dark-bg)] pb-20 selection:bg-blue-100 selection:text-blue-900 overflow-x-hidden">
      ...
      <ThreeBackground isDarkMode={isDarkMode} className="fixed inset-0 pointer-events-none z-0 opacity-70" />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-full flex items-center justify-between gap-4 min-w-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto no-scrollbar py-1 shrink min-w-0 max-w-full">
            ...
            {userStats?.isPro ? (
              ...
              <button
                onClick={() => setIsUpgradeModalOpen(true)}
                className="shrink-0 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 border border-blue-500/20 text-blue-600 dark:text-blue-400 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 whitespace-nowrap"
              >
                <Zap className="w-3.5 h-3.5 text-amber-500 fill-amber-500" />
                <span>{Math.max(0, (5 + (userStats?.bonusAnalyses || 0)) - (userStats?.analysesUsed || 0))} free</span>
              </button>
            )}
            ...
            <div className="shrink-0 flex items-center gap-2">
              {(isAdmin || currentUser?.email?.toLowerCase() === 'ayaicrypcoin@gmail.com') && (
                <button
                  onClick={() => setView('admin')}
                  className={cn(
                    "px-3 py-2 border rounded-2xl transition-all text-xs font-black flex items-center gap-1.5 cursor-pointer shadow-xs whitespace-nowrap",
                    view === 'admin'
      ...
      <main className={cn("mx-auto px-4 md:px-6 pt-8 md:pt-12 transition-all", (view === 'notebooks' || view === 'admin') ? "max-w-7xl" : "max-w-5xl")}>
        {authLoading ? (
          ...
          {!result ? (
            ...
            <AnimatePresence>
               {loading && (
                 <motion.div 
                   ...
                   animate={{ opacity: 1 }}
                   exit={{ opacity: 0 }}
                   className="fixed inset-0 z-[100] bg-[#FDFCFB]/80 dark:bg-slate-950/80 backdrop-blur-3xl flex items-center justify-center p-6"
                 >
                   <div className="max-w-4xl w-full space-y-12">
                      <div className="text-center space-y-6">
                         <div className="w-28 h-28 bg-slate-950 dark:bg-white rounded-[3rem] flex items-center justify-center mx-auto shadow-deep animate-pulse">
                           <Brain className="w-12 h-12 text-white dark:text-slate-950" />
                         </div>
                         <h3 className="text-5xl md:text-6xl font-display font-black text-slate-950 dark:text-white tracking-tighter leading-tight max-w-2xl mx-auto">
                            Generating Intelligence Layer...
                         </h3>
                         <div className="flex items-center justify-center gap-3">
                            <div className="w-2 h-2 rounded-full bg-blue-600 animate-bounce" />
                            <p className="text-slate-500 dark:text-slate-400 font-black uppercase tracking-[0.5em] text-[10px]">{loadingMessage}</p>
                         </div>
                      </div>
                      
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                         {[1,2,3].map(i => (
                           <div key={`skeleton-${i}`} className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-lush space-y-8 animate-pulse">
                              <div className="h-14 w-14 bg-slate-100 dark:bg-slate-800 rounded-2xl" />
                              <div className="space-y-4">
                                 <div className="h-6 w-3/4 bg-slate-100 dark:bg-slate-800 rounded-full" />
                                 <div className="h-4 w-1/2 bg-slate-50 dark:bg-slate-800/50 rounded-full" />
                              </div>
                              <div className="space-y-3">
                                 <div className="h-2 w-full bg-slate-50 dark:bg-slate-800/30 rounded-full" />
                                 <div className="h-2 w-full bg-slate-50 dark:bg-slate-800/30 rounded-full" />
                                 <div className="h-2 w-2/3 bg-slate-50 dark:bg-slate-800/30 rounded-full" />
                              </div>
                           </div>
                         ))}
                      </div>
            ...
            >
              ...
              <div className="bg-slate-950 rounded-[4rem] p-10 md:p-16 text-white shadow-deep relative overflow-hidden group border border-white/5 card-shine">
                ...
                <div className="relative z-20 mb-12">
                  <button 
                    onClick={reset}
                    className="group/back inline-flex items-center gap-3 px-6 py-2 rounded-full glass-morphism border border-white/10 hover:bg-white/10 transition-all"
                  >
                    <ArrowLeft className="w-5 h-5 group-hover/back:-translate-x-2 transition-transform text-blue-400" />
                    <span className="font-display font-black text-[10px] uppercase tracking-[0.3em] text-slate-300">Back to Terminal</span>
                  </button>
                </div>
                ...
                <div className="relative z-10 flex flex-col xl:flex-row xl:items-end justify-between gap-16">
                  <div className="space-y-8 max-w-3xl">
                    <div className="flex flex-wrap gap-3">
                      <div className="px-5 py-2 bg-blue-600 rounded-xl text-[10px] font-black tracking-[0.3em] uppercase shadow-deep">
                        {result.subject}
                      </div>
                      {(result.topics || []).slice(0, 3).map((t, idx) => (
                        <div key={`hero-topic-${idx}-${t}`} className="px-5 py-2 bg-white/5 backdrop-blur-3xl rounded-xl text-[10px] font-black tracking-[0.3em] uppercase border border-white/10 text-slate-400 whitespace-nowrap">
                          {t}
                        </div>
                      ))}
                    </div>
                    
                    <h2 className="text-6xl md:text-8xl font-display font-black tracking-tighter leading-[0.8] text-white">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-1 gap-4 w-full xl:w-80">
                    <button 
                      ...
                      className="bg-indigo-600 text-white p-6 rounded-3xl font-display font-black transition-all flex items-center justify-center gap-4 shadow-deep hover:scale-[1.02] active:scale-95 group/export relative overflow-hidden"
                    >
                      <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover/export:translate-x-full transition-transform duration-1000" />
                      <Download className="w-6 h-6 group-hover:animate-bounce" /> 
                      <span className="uppercase text-xs tracking-widest">Export Protocol</span>
                    </button>
                    <button 
                      onClick={() => setView('dashboard')}
                      className="bg-white/5 backdrop-blur-3xl text-white border border-white/10 p-6 rounded-3xl font-display font-black transition-all flex items-center justify-center gap-4 hover:bg-white/10 active:scale-95 group/dash"
                    >
                      <History className="w-6 h-6 text-slate-400 group-hover:text-white transition-colors" /> 
                      <span className="uppercase text-xs tracking-widest">Access Log</span>
                    </button>
                    <button 
                      onClick={() => {
                        if (!userStats?.isPro) {
                      ...
                      className="bg-white text-slate-950 p-6 rounded-3xl font-display font-black transition-all flex items-center justify-center gap-4 shadow-deep hover:scale-[1.02] active:scale-95 group/voice cursor-pointer"
                    >
                      <Mic className="w-6 h-6 group-hover:animate-pulse" /> 
                      <span className="uppercase text-xs tracking-widest">Neural Link</span>
                      {!userStats?.isPro && (
        ...
        <AnimatePresence>
            ...
            {showResumeMaker && (
              <ResumeMaker onClose={() => setShowResumeMaker(false)} />
            )}
            {showWeaknessDetector && (
              <WeaknessDetector onClose={() => setShowWeaknessDetector(false)} />
            )}
            {showImageStudio && (
              <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-md overflow-y-auto p-4 md:p-8 flex justify-center items-start">
      ...
      <footer className="mt-20 text-center text-slate-400 text-sm py-12 border-t dark:border-slate-800 bg-white dark:bg-slate-950">
        ...
        <p className="font-black tracking-widest uppercase text-[10px] dark:text-slate-500">AI Study BUDDY by Ayan Ahmed</p>
